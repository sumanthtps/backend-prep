...
annotations: | Annotation | Purpose | |------------|---------| | `@Component` |
General-purpose Spring-managed bean. | | `@Service` | Used for **service layer**
classes (business logic). | | `@Repository` | Used for **DAO (Data Access
Objects)** interacting with databases. | | `@Controller/@RestController` | Used
for **handling HTTP requests** in web applications. |
...
There are two types: | **Type** | **Description** |
|----------|----------------| | **ApplicationContext** ✅ | Most commonly used
IoC container. | | **BeanFactory** ❌ | Lower-level container, rarely used. |
